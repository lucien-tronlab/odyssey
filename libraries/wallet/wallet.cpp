...
#include <odyssey/wallet/wallet.hpp>
#include <odyssey/wallet/api_documentation.hpp>
#include <odyssey/wallet/reflect_util.hpp>

#include <odyssey/account_by_key/account_by_key_api.hpp>
...
public:
   ...
   variant info() const
   {
      ...
      result["post_reward_fund"] = fc::variant(_remote_db->get_reward_fund( ODYSSEY_POST_REWARD_FUND_NAME )).get_object();
      return result;
   }
   ...
   variant_object about() const
   {
      ...
      result["blockchain_version"]       = ODYSSEY_BLOCKCHAIN_VERSION;
      result["client_version"]           = client_version;
      result["odyssey_revision"]           = graphene::utilities::git_revision_sha;
      result["odyssey_revision_age"]       = fc::get_approximate_relative_time_string( fc::time_point_sec( graphene::utilities::git_revision_unix_timestamp ) );
      ...
      try
      {
         ...
         result["server_odyssey_revision"] = v.odyssey_revision;
   ...
   bool import_key(string wif_key)
   {
      ...
      odyssey::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         odyssey::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   { try {
         ...
         odyssey::chain::public_key_type owner_pubkey = owner_privkey.get_public_key();
         odyssey::chain::public_key_type active_pubkey = active_privkey.get_public_key();
         odyssey::chain::public_key_type memo_pubkey = memo_privkey.get_public_key();
   ...
   {
      FC_ASSERT( tx_expiration_seconds < ODYSSEY_MAX_TIME_UNTIL_EXPIRATION );
   ...
   annotated_signed_transaction sign_transaction(signed_transaction tx, bool broadcast = false)
   {
      ...
      for( const public_key_type& k : minimal_signing_keys )
      {
         ...
         tx.sign( it->second, ODYSSEY_CHAIN_ID );
      }
   ...
   std::map<string,std::function<string(fc::variant,const fc::variants&)>> get_result_formatters() const
   {
      ...
      m["list_my_accounts"] = [](variant result, const fc::variants& a ) {
         ...
         out << "-------------------------------------------------------------------------\n";
            out << std::left << std::setw( 17 ) << "TOTAL"
                << std::right << std::setw(18) << fc::variant(total_odyssey).as_string() <<" "
      ...
      m["get_open_orders"] = []( variant result, const fc::variants& a ) {
          ...
          for( const auto& o : orders ) {
             ...
             ss << ' ' << setw( 10 ) << (o.sell_price.base.symbol == ODYSSEY_SYMBOL ? "SELL" : "BUY");
             ss << "\n";
          }
          return ss.str();
      };
         ...
         for( size_t i = 0; i < orders.bids.size() || i < orders.asks.size(); i++ )
         {
            if ( i < orders.bids.size() )
            {
               ...
               ss
                  ...
                  << ' ' << setw( spacing ) << asset( orders.bids[i].odyssey, ODYSSEY_SYMBOL ).to_string()
            ...
            if ( i < orders.asks.size() )
            {
               ...
               ss << ' ' << setw( spacing ) << orders.asks[i].real_price
                  << ' ' << setw( spacing ) << asset( orders.asks[i].odyssey, ODYSSEY_SYMBOL ).to_string()
...
{ try {
   ...
   op.fee = my->_remote_db->get_chain_properties().account_creation_fee * asset( ODYSSEY_CREATE_ACCOUNT_WITH_ODYSSEY_MODIFIER, ODYSSEY_SYMBOL );
 ...
 *  will be controlable by this wallet.
 */
annotated_signed_transaction wallet_api::create_account_delegated( string creator, asset odyssey_fee, asset delegated_vests, string new_account_name, string json_meta, bool broadcast )
{ try {
   FC_ASSERT( !is_locked() );
   auto owner = suggest_brain_key();
   auto active = suggest_brain_key();
   ...
   import_key( posting.wif_priv_key );
   import_key( memo.wif_priv_key );
   return create_account_with_keys_delegated( creator, odyssey_fee, delegated_vests, new_account_name, json_meta,  owner.pub_key, active.pub_key, posting.pub_key, memo.pub_key, broadcast );
} FC_CAPTURE_AND_RETHROW( (creator)(new_account_name)(json_meta) ) }
   ...
   )
{
   ...
   op.odyssey_amount = odyssey_amount;
   op.fee = fee;
...
annotated_signed_transaction wallet_api::claim_reward_balance( string account, asset reward_odyssey, asset reward_sbd, asset reward_vests, bool broadcast )
{
   FC_ASSERT( !is_locked() );
   claim_reward_balance_operation op;
   op.account = account;
   op.reward_odyssey = reward_odyssey;
...
annotated_signed_transaction wallet_api::vote( string voter, string author, string permlink, int16_t weight, bool broadcast )
{
   ...
   op.author = author;
   op.permlink = permlink;
   op.weight = weight * ODYSSEY_1_PERCENT;
...
annotated_signed_transaction      wallet_api::send_private_message( string from, string to, string subject, string body, bool broadcast ) {
   ...
   op.id = ODYSSEY_PRIVATE_MESSAGE_COP_ID;
