...
#include <odyssey/app/database_api.hpp>
...
{
   public:
      database_api_impl( const odyssey::app::api_context& ctx  );
      ~database_api_impl();
      ...
      odyssey::chain::database&                _db;
      std::shared_ptr< odyssey::follow::follow_api > _follow_api;
...
database_api::database_api( const odyssey::app::api_context& ctx )
   : my( new database_api_impl( ctx ) ) {}

database_api::~database_api() {}

database_api_impl::database_api_impl( const odyssey::app::api_context& ctx )
   : _db( *ctx.app.chain_database() )
{
   ...
   try
   {
      ...
      _follow_api = std::make_shared< odyssey::follow::follow_api >( ctx );
   }
...
{
   return odyssey::protocol::get_config();
}
...
vector<account_id_type> database_api_impl::get_account_references( account_id_type account_id )const
{
   ...
   const auto& refs = aidx.get_secondary_index<odyssey::chain::account_member_index>();
   ...
   return result;*/
   FC_ASSERT( false, "database_api::get_account_references --- Needs to be refactored for odyssey." );
}
...
order_book database_api_impl::get_order_book( uint32_t limit )const
{
   ...
   auto max_sell = price::max( SBD_SYMBOL, ODYSSEY_SYMBOL );
   auto max_buy = price::max( ODYSSEY_SYMBOL, SBD_SYMBOL );
...
//   if( buy_itr != end ) idump((*buy_itr));
   ...
   while(  sell_itr != end && sell_itr->sell_price.base.symbol == SBD_SYMBOL && result.bids.size() < limit )
   {
      ...
      cur.odyssey = ( asset( itr->for_sale, SBD_SYMBOL ) * cur.order_price ).amount;
      cur.created = itr->created;
      result.bids.push_back( cur );
      ++sell_itr;
   }
   while(  buy_itr != end && buy_itr->sell_price.base.symbol == ODYSSEY_SYMBOL && result.asks.size() < limit )
   {
      auto itr = buy_itr;
      order cur;
      cur.order_price = itr->sell_price;
      cur.real_price  = (~cur.order_price).to_real();
      cur.odyssey   = itr->for_sale;
      cur.sbd     = ( asset( itr->for_sale, ODYSSEY_SYMBOL ) * cur.order_price ).amount;
      cur.created = itr->created;
...
//   wdump((trx)(available_keys));
   auto result = trx.get_required_signatures( ODYSSEY_CHAIN_ID,
                                              available_keys,
...
{
   trx.verify_authority( ODYSSEY_CHAIN_ID,
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).active  ); },
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).owner   ); },
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).posting ); },
                         ODYSSEY_MAX_SIG_CHECK_DEPTH );
...
void database_api::set_pending_payout( discussion& d )const
{
   ...
   asset pot;
   if( my->_db.has_hardfork( ODYSSEY_HARDFORK_0_17__774 ) )
      pot = my->_db.get_reward_fund( my->_db.get_comment( d.author, d.permlink ) ).reward_balance;
   else
      pot = props.total_reward_fund_odyssey;
   ...
   if( total_r2 > 0 )
   {
      ...
      if( my->_db.has_hardfork( ODYSSEY_HARDFORK_0_17__774 ) )
      {
         ...
         vshares = d.net_rshares.value > 0 ? odyssey::chain::util::evaluate_reward_curve( d.net_rshares.value, rf.author_reward_curve, rf.content_constant ) : 0;
      }
      else
         vshares = d.net_rshares.value > 0 ? odyssey::chain::util::evaluate_reward_curve( d.net_rshares.value ) : 0;
   ...
   if( d.parent_author != ODYSSEY_ROOT_POST_PARENT )
...
vector<discussion> database_api::get_discussions_by_promoted( const discussion_query& query )const
{
   return my->_db.with_read_lock( [&]()
   {
      ...
      auto tidx_itr = tidx.lower_bound( boost::make_tuple( tag, parent, share_type(ODYSSEY_MAX_SHARE_SUPPLY) )  );
