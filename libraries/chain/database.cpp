#include <odyssey/protocol/odyssey_operations.hpp>

#include <odyssey/chain/block_summary_object.hpp>
#include <odyssey/chain/compound.hpp>
#include <odyssey/chain/custom_operation_interpreter.hpp>
#include <odyssey/chain/database.hpp>
#include <odyssey/chain/database_exceptions.hpp>
#include <odyssey/chain/db_with.hpp>
#include <odyssey/chain/evaluator_registry.hpp>
#include <odyssey/chain/global_property_object.hpp>
#include <odyssey/chain/history_object.hpp>
#include <odyssey/chain/index.hpp>
#include <odyssey/chain/odyssey_evaluator.hpp>
#include <odyssey/chain/odyssey_objects.hpp>
#include <odyssey/chain/transaction_object.hpp>
#include <odyssey/chain/shared_db_merkle.hpp>
#include <odyssey/chain/operation_notification.hpp>
#include <odyssey/chain/witness_schedule.hpp>

#include <odyssey/chain/util/asset.hpp>
#include <odyssey/chain/util/reward.hpp>
#include <odyssey/chain/util/uint256.hpp>
#include <odyssey/chain/util/reward.hpp>
...
FC_REFLECT( odyssey::chain::object_schema_repr, (space_type)(type) )
FC_REFLECT( odyssey::chain::operation_schema_repr, (id)(type) )
FC_REFLECT( odyssey::chain::db_schema, (types)(object_types)(operation_type)(custom_operation_types) )
...
struct reward_fund_context
{
   ...
   asset       reward_balance = asset( 0, ODYSSEY_SYMBOL );
   share_type  odyssey_awarded = 0;
};
...
void database::reindex( const fc::path& data_dir, const fc::path& shared_mem_dir, uint64_t shared_file_size )
{
   try
   {
      ...
      ODYSSEY_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
...
{ try {
   if( !has_hardfork( ODYSSEY_HARDFORK_0_6__127 ) )
...
{
   if( has_hardfork( ODYSSEY_HARDFORK_0_17__769 ) || comment.parent_author == ODYSSEY_ROOT_POST_PARENT )
...
{
   return get< reward_fund_object, by_name >( ODYSSEY_POST_REWARD_FUND_NAME );
}
...
uint32_t database::witness_participation_rate()const
{
   ...
   return uint64_t(ODYSSEY_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
   ...
   )
{
   ...
   auto maximum_block_size = get_dynamic_global_properties().maximum_block_size; //ODYSSEY_MAX_BLOCK_SIZE;
   ...
   if( has_hardfork( ODYSSEY_HARDFORK_0_5__54 ) )
   {
      const auto& witness = get_witness( witness_owner );

      if( witness.running_version != ODYSSEY_BLOCKCHAIN_VERSION )
         pending_block.extensions.insert( block_header_extensions( ODYSSEY_BLOCKCHAIN_VERSION ) );

      const auto& hfp = get_hardfork_property_object();

      if( hfp.current_hardfork_version < ODYSSEY_BLOCKCHAIN_HARDFORK_VERSION // Binary is newer hardfork than has been applied
      ...
      else if( hfp.current_hardfork_version == ODYSSEY_BLOCKCHAIN_HARDFORK_VERSION // Binary does not know of a new hardfork
         && witness.hardfork_version_vote > ODYSSEY_BLOCKCHAIN_HARDFORK_VERSION ) // Voting for hardfork in the future, that we do not know of...
      {
   ...
   {
      FC_ASSERT( fc::raw::pack_size(pending_block) <= ODYSSEY_MAX_BLOCK_SIZE );
   }
...
void database::pop_block()
{
   try
   {
      ...
      ODYSSEY_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
void database::notify_pre_apply_operation( operation_notification& note )
{
   ...
   ODYSSEY_TRY_NOTIFY( pre_apply_operation, note )
}

void database::notify_post_apply_operation( const operation_notification& note )
{
   ODYSSEY_TRY_NOTIFY( post_apply_operation, note )
}
...
{
   ODYSSEY_TRY_NOTIFY( applied_block, block )
}

void database::notify_on_pending_transaction( const signed_transaction& tx )
{
   ODYSSEY_TRY_NOTIFY( on_pending_transaction, tx )
}

void database::notify_on_pre_apply_transaction( const signed_transaction& tx )
{
   ODYSSEY_TRY_NOTIFY( on_pre_apply_transaction, tx )
}

void database::notify_on_applied_transaction( const signed_transaction& tx )
{
   ODYSSEY_TRY_NOTIFY( on_applied_transaction, tx )
}
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = ODYSSEY_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / ODYSSEY_BLOCK_INTERVAL + 1;
}

/**
 *  Converts ODYSSEY into sbd and adds it to to_account while reducing the ODYSSEY supply
 *  by ODYSSEY and increasing the sbd supply by the specified amount.
 */
std::pair< asset, asset > database::create_sbd( const account_object& to_account, asset odyssey, bool to_reward_balance )
{
   std::pair< asset, asset > assets( asset( 0, SBD_SYMBOL ), asset( 0, ODYSSEY_SYMBOL ) );
   ...
   try
   {
      ...
      {
         auto to_sbd = ( gpo.sbd_print_rate * odyssey.amount ) / ODYSSEY_100_PERCENT;
         auto to_odyssey = odyssey.amount - to_sbd;

         auto sbd = asset( to_sbd, ODYSSEY_SYMBOL ) * median_price;

         if( to_reward_balance )
         {
            adjust_reward_balance( to_account, sbd );
            adjust_reward_balance( to_account, asset( to_odyssey, ODYSSEY_SYMBOL ) );
         }
         else
         {
            adjust_balance( to_account, sbd );
            adjust_balance( to_account, asset( to_odyssey, ODYSSEY_SYMBOL ) );
         }

         adjust_supply( asset( -to_sbd, ODYSSEY_SYMBOL ) );
         adjust_supply( sbd );
         assets.first = sbd;
         assets.second = to_odyssey;
      }
      else
      {
         adjust_balance( to_account, odyssey );
         assets.second = odyssey;
      }
   }
   FC_CAPTURE_LOG_AND_RETHROW( (to_account.name)(odyssey) )
 ...
 * @param ODYSSEY - ODYSSEY to be converted to vesting shares
 */
asset database::create_vesting( const account_object& to_account, asset odyssey, bool to_reward_balance )
{
   try
   {
      ...
      /**
       *  The ratio of total_vesting_shares / total_vesting_fund_odyssey should not
       ...
       */
      asset new_vesting = odyssey * ( to_reward_balance ? cprops.get_reward_vesting_share_price() : cprops.get_vesting_share_price() );
      ...
      modify( to_account, [&]( account_object& to )
      {
         if( to_reward_balance )
         {
            ...
            to.reward_vesting_odyssey += odyssey;
         }
         else
      ...
      modify( cprops, [&]( dynamic_global_property_object& props )
      {
         if( to_reward_balance )
         {
            ...
            props.pending_rewarded_vesting_odyssey += odyssey;
         }
         else
         {
            props.total_vesting_fund_odyssey += odyssey;
   ...
   FC_CAPTURE_AND_RETHROW( (to_account.name)(odyssey) )
}
...
uint32_t database::get_pow_summary_target()const
{
   ...
   if( has_hardfork( ODYSSEY_HARDFORK_0_16__551 ) )
...
void database::adjust_proxied_witness_votes( const account_object& a,
                                   const std::array< share_type, ODYSSEY_MAX_PROXY_RECURSION_DEPTH+1 >& delta,
                                   int depth )
{
   if( a.proxy != ODYSSEY_PROXY_TO_SELF_ACCOUNT )
   {
      /// nested proxies are not supported, vote will not propagate
      if( depth >= ODYSSEY_MAX_PROXY_RECURSION_DEPTH )
         return;
      ...
      {
         for( int i = ODYSSEY_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
         {
   ...
   else
   {
      ...
      for( int i = ODYSSEY_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
         total_delta += delta[i];
...
void database::adjust_witness_vote( const witness_object& witness, share_type delta )
{
   ...
   modify( witness, [&]( witness_object& w )
   {
      ...
      if( has_hardfork( ODYSSEY_HARDFORK_0_2 ) )
      ...
      if( has_hardfork( ODYSSEY_HARDFORK_0_4 ) )
      {
...
void database::clear_witness_votes( const account_object& a )
{
   ...
   if( has_hardfork( ODYSSEY_HARDFORK_0_6__104 ) )
...
{
   if( !has_hardfork( ODYSSEY_HARDFORK_0_14__327 ) ) return;

   const auto& null_account = get_account( ODYSSEY_NULL_ACCOUNT );
   asset total_odyssey( 0, ODYSSEY_SYMBOL );
   asset total_sbd( 0, SBD_SYMBOL );

   if( null_account.balance.amount > 0 )
   {
      total_odyssey += null_account.balance;
      adjust_balance( null_account, -null_account.balance );
   }

   if( null_account.savings_balance.amount > 0 )
   {
      total_odyssey += null_account.savings_balance;
   ...
   {
      const auto& gpo = get_dynamic_global_properties();
      auto converted_odyssey = null_account.vesting_shares * gpo.get_vesting_share_price();

      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         g.total_vesting_shares -= null_account.vesting_shares;
         g.total_vesting_fund_odyssey -= converted_odyssey;
      });
      ...
      total_odyssey += converted_odyssey;
   }

   if( null_account.reward_odyssey_balance.amount > 0 )
   {
      total_odyssey += null_account.reward_odyssey_balance;
      adjust_reward_balance( null_account, -null_account.reward_odyssey_balance );
   }
   ...
   if( null_account.reward_vesting_balance.amount > 0 )
   {
      ...
      total_odyssey += null_account.reward_vesting_odyssey;

      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         g.pending_rewarded_vesting_shares -= null_account.reward_vesting_balance;
         g.pending_rewarded_vesting_odyssey -= null_account.reward_vesting_odyssey;
      });

      modify( null_account, [&]( account_object& a )
      {
         a.reward_vesting_odyssey.amount = 0;
         a.reward_vesting_balance.amount = 0;
      });
   }

   if( total_odyssey.amount > 0 )
      adjust_supply( -total_odyssey );
...
{
   if( head_block_num() >= ODYSSEY_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
   {
...
void database::process_vesting_withdrawals()
{
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
      ...
      share_type vests_deposited_as_odyssey = 0;
      share_type vests_deposited_as_vests = 0;
      asset total_odyssey_converted = asset( 0, ODYSSEY_SYMBOL );

      // Do two passes, the first for vests, the second for odyssey. Try to maintain as much accuracy for vests as possible.
           ...
           ++itr )
      {
         if( itr->auto_vest )
         {
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / ODYSSEY_100_PERCENT ).to_uint64();
           ...
           ++itr )
      {
         if( !itr->auto_vest )
         {
            ...
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / ODYSSEY_100_PERCENT ).to_uint64();
            vests_deposited_as_odyssey += to_deposit;
            auto converted_odyssey = asset( to_deposit, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
            total_odyssey_converted += converted_odyssey;

            if( to_deposit > 0 )
            {
               modify( to_account, [&]( account_object& a )
               {
                  a.balance += converted_odyssey;
               });

               modify( cprops, [&]( dynamic_global_property_object& o )
               {
                  o.total_vesting_fund_odyssey -= converted_odyssey;
                  o.total_vesting_shares.amount -= to_deposit;
               });

               push_virtual_operation( fill_vesting_withdraw_operation( from_account.name, to_account.name, asset( to_deposit, VESTS_SYMBOL), converted_odyssey ) );
            }
         }
      }

      share_type to_convert = to_withdraw - vests_deposited_as_odyssey - vests_deposited_as_vests;
      FC_ASSERT( to_convert >= 0, "Deposited more vests than were supposed to be withdrawn" );

      auto converted_odyssey = asset( to_convert, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.next_vesting_withdrawal += fc::seconds( ODYSSEY_VESTING_WITHDRAW_INTERVAL_SECONDS );
         }
      });
      ...
      push_virtual_operation( fill_vesting_withdraw_operation( from_account.name, from_account.name, asset( to_withdraw, VESTS_SYMBOL ), converted_odyssey ) );
   }
}
      ...
      else if( c.total_vote_weight > 0 )
      {
         ...
         while( itr != cvidx.end() && itr->comment == c.id )
         {
            ...
            if( claim > 0 ) // min_amt is non-zero satoshis
            {
               ...
               auto reward = create_vesting( voter, asset( claim, ODYSSEY_SYMBOL ), has_hardfork( ODYSSEY_HARDFORK_0_17__659 ) );
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment )
{
   try
   {
      ...
      if( comment.net_rshares > 0 )
      {
         ...
         if( has_hardfork( ODYSSEY_HARDFORK_0_17__774 ) )
         {
         ...
         if( reward_tokens > 0 )
         {
            share_type curation_tokens = ( ( reward_tokens * get_curation_rewards_percent( comment ) ) / ODYSSEY_100_PERCENT ).to_uint64();
            ...
            {
               auto benefactor_tokens = ( author_tokens * b.weight ) / ODYSSEY_100_PERCENT;
               auto vest_created = create_vesting( get_account( b.account ), benefactor_tokens, has_hardfork( ODYSSEY_HARDFORK_0_17__659 ) );
            ...
            auto sbd_odyssey     = ( author_tokens * comment.percent_odyssey_dollars ) / ( 2 * ODYSSEY_100_PERCENT ) ;
            auto vesting_odyssey = author_tokens - sbd_odyssey;

            const auto& author = get_account( comment.author );
            auto vest_created = create_vesting( author, vesting_odyssey, has_hardfork( ODYSSEY_HARDFORK_0_17__659 ) );
            auto sbd_payout = create_sbd( author, sbd_odyssey, has_hardfork( ODYSSEY_HARDFORK_0_17__659 ) );

            adjust_total_payout( comment, sbd_payout.first + to_sbd( sbd_payout.second + asset( vesting_odyssey, ODYSSEY_SYMBOL ) ), to_sbd( asset( curation_tokens, ODYSSEY_SYMBOL ) ), to_sbd( asset( total_beneficiary, ODYSSEY_SYMBOL ) ) );
            ...
            push_virtual_operation( comment_reward_operation( comment.author, to_string( comment.permlink ), to_sbd( asset( claimed_reward, ODYSSEY_SYMBOL ) ) ) );
      ...
      modify( comment, [&]( comment_object& c )
      {
         ...
         */
         ...
         if( has_hardfork( ODYSSEY_HARDFORK_0_17__769 ) )
         {
            c.cashout_time = fc::time_point_sec::maximum();
         }
         else if( c.parent_author == ODYSSEY_ROOT_POST_PARENT )
         {
            if( has_hardfork( ODYSSEY_HARDFORK_0_12__177 ) && c.last_payout == fc::time_point_sec::min() )
               c.cashout_time = head_block_time() + ODYSSEY_SECOND_CASHOUT_WINDOW;
            else
               c.cashout_time = fc::time_point_sec::maximum();
         }
      ...
      while( vote_itr != vote_idx.end() && vote_itr->comment == comment.id )
      {
         ...
         ++vote_itr;
         if( !has_hardfork( ODYSSEY_HARDFORK_0_12__177 ) || calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum() )
         {
...
void database::process_comment_cashout()
{
   ...
   if( !has_hardfork( ODYSSEY_FIRST_CASHOUT_TIME ) )
      return;

   const auto& gpo = get_dynamic_global_properties();
   util::comment_reward_context ctx;
   ctx.current_odyssey_price = get_feed_history().current_median_history;

   vector< reward_fund_context > funds;
   vector< share_type > odyssey_awarded;
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      modify( *itr, [&]( reward_fund_object& rfo )
      {
         ...
         if( has_hardfork( ODYSSEY_HARDFORK_0_19__1051 ) )
            decay_rate = ODYSSEY_RECENT_RSHARES_DECAY_RATE_HF19;
         else
            decay_rate = ODYSSEY_RECENT_RSHARES_DECAY_RATE_HF17;
   ...
   if( has_hardfork( ODYSSEY_HARDFORK_0_17__771 ) )
   {
   ...
   while( current != cidx.end() && current->cashout_time <= head_block_time() )
   {
      if( has_hardfork( ODYSSEY_HARDFORK_0_17__771 ) )
      {
         ...
         ctx.total_reward_fund_odyssey = funds[ fund_id ].reward_balance;
         funds[ fund_id ].odyssey_awarded += cashout_comment_helper( ctx, *current );
      }
      else
      {
         ...
         while( itr != com_by_root.end() && itr->root_comment == current->root_comment )
         {
            ...
            ctx.total_reward_fund_odyssey = gpo.total_reward_fund_odyssey;
            ...
            if( reward > 0 )
            {
               ...
               {
                  p.total_reward_fund_odyssey.amount -= reward;
               });
            }
         }
      }
   ...
   if( funds.size() )
   {
      for( size_t i = 0; i < funds.size(); i++ )
      {
         modify( get< reward_fund_object, by_id >( reward_fund_id_type( i ) ), [&]( reward_fund_object& rfo )
         {
            ...
            rfo.reward_balance -= funds[ i ].odyssey_awarded;
         });
      }
   }
}
...
/**
 *  Overall the network has an inflation rate of 102% of virtual odyssey per year
...
void database::process_funds()
{
   ...
   if( has_hardfork( ODYSSEY_HARDFORK_0_16__551) )
   {
       ...
       */
      int64_t start_inflation_rate = int64_t( ODYSSEY_INFLATION_RATE_START_PERCENT );
      int64_t inflation_rate_adjustment = int64_t( head_block_num() / ODYSSEY_INFLATION_NARROWING_PERIOD );
      int64_t inflation_rate_floor = int64_t( ODYSSEY_INFLATION_RATE_STOP_PERCENT );

      // below subtraction cannot underflow int64_t because inflation_rate_adjustment is <2^32
      int64_t current_inflation_rate = std::max( start_inflation_rate - inflation_rate_adjustment, inflation_rate_floor );

      auto new_odyssey = ( props.virtual_supply.amount * current_inflation_rate ) / ( int64_t( ODYSSEY_100_PERCENT ) * int64_t( ODYSSEY_BLOCKS_PER_YEAR ) );
      auto content_reward = ( new_odyssey * ODYSSEY_CONTENT_REWARD_PERCENT ) / ODYSSEY_100_PERCENT;
      if( has_hardfork( ODYSSEY_HARDFORK_0_17__774 ) )
         content_reward = pay_reward_funds( content_reward ); /// 75% to content creator
      auto vesting_reward = ( new_odyssey * ODYSSEY_VESTING_FUND_PERCENT ) / ODYSSEY_100_PERCENT; /// 15% to vesting fund
      auto witness_reward = new_odyssey - content_reward - vesting_reward; /// Remaining 10% to witness pay

      const auto& cwit = get_witness( props.current_witness );
      witness_reward *= ODYSSEY_MAX_WITNESSES;
      ...
      new_odyssey = content_reward + vesting_reward + witness_reward;

      modify( props, [&]( dynamic_global_property_object& p )
      {
         p.total_vesting_fund_odyssey += asset( vesting_reward, ODYSSEY_SYMBOL );
         if( !has_hardfork( ODYSSEY_HARDFORK_0_17__774 ) )
            p.total_reward_fund_odyssey  += asset( content_reward, ODYSSEY_SYMBOL );
         p.current_supply           += asset( new_odyssey, ODYSSEY_SYMBOL );
         p.virtual_supply           += asset( new_odyssey, ODYSSEY_SYMBOL );
      });

      const auto& producer_reward = create_vesting( get_account( cwit.owner ), asset( witness_reward, ODYSSEY_SYMBOL ) );
   ...
   else
   {
      ...
      if( props.head_block_number < ODYSSEY_START_VESTING_BLOCK )
      ...
      modify( props, [&]( dynamic_global_property_object& p )
      {
          p.total_vesting_fund_odyssey += vesting_reward;
          p.total_reward_fund_odyssey  += content_reward;
...
{
   if( has_hardfork( ODYSSEY_HARDFORK_0_12__178 ) )
      return asset( 0, ODYSSEY_SYMBOL );

   const auto& props = get_dynamic_global_properties();
   static_assert( ODYSSEY_LIQUIDITY_REWARD_PERIOD_SEC == 60*60, "this code assumes a 1 hour time interval" );
   asset percent( protocol::calc_percent_reward_per_hour< ODYSSEY_LIQUIDITY_APR_PERCENT >( props.virtual_supply.amount ), ODYSSEY_SYMBOL );
   return std::max( percent, ODYSSEY_MIN_LIQUIDITY_REWARD );
}

asset database::get_content_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( ODYSSEY_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< ODYSSEY_CONTENT_APR_PERCENT >( props.virtual_supply.amount ), ODYSSEY_SYMBOL );
   return std::max( percent, ODYSSEY_MIN_CONTENT_REWARD );
}
...
asset database::get_curation_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( ODYSSEY_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< ODYSSEY_CURATE_APR_PERCENT >( props.virtual_supply.amount ), ODYSSEY_SYMBOL);
   return std::max( percent, ODYSSEY_MIN_CURATE_REWARD );
}
...
asset database::get_producer_reward()
{
   const auto& props = get_dynamic_global_properties();
   static_assert( ODYSSEY_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< ODYSSEY_PRODUCER_APR_PERCENT >( props.virtual_supply.amount ), ODYSSEY_SYMBOL);
   auto pay = std::max( percent, ODYSSEY_MIN_PRODUCER_REWARD );
   const auto& witness_account = get_account( props.current_witness );

   /// pay witness in vesting shares
   if( props.head_block_number >= ODYSSEY_START_MINER_VOTING_BLOCK || (witness_account.vesting_shares.amount.value == 0) ) {
...
#ifndef IS_TEST_NET
   /// 0 block rewards until at least ODYSSEY_MAX_WITNESSES have produced a POW
   if( props.num_pow_witnesses < ODYSSEY_MAX_WITNESSES && props.head_block_number < ODYSSEY_START_VESTING_BLOCK )
      return asset( 0, ODYSSEY_SYMBOL );
#endif

   static_assert( ODYSSEY_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   static_assert( ODYSSEY_MAX_WITNESSES == 21, "this code assumes 21 per round" );
   asset percent( calc_percent_reward_per_round< ODYSSEY_POW_APR_PERCENT >( props.virtual_supply.amount ), ODYSSEY_SYMBOL);
   return std::max( percent, ODYSSEY_MIN_POW_REWARD );
}
...
void database::pay_liquidity_reward()
{
   ...
   if( (head_block_num() % ODYSSEY_LIQUIDITY_REWARD_BLOCKS) == 0 )
   {
...
uint16_t database::get_curation_rewards_percent( const comment_object& c ) const
{
   ...
   else if( has_hardfork( ODYSSEY_HARDFORK_0_8__116 ) )
      return ODYSSEY_1_PERCENT * 25;
   else
      return ODYSSEY_1_PERCENT * 50;
}
...
share_type database::pay_reward_funds( share_type reward )
{
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      auto r = ( reward * itr->percent_content_rewards ) / ODYSSEY_100_PERCENT;

      modify( *itr, [&]( reward_fund_object& rfo )
      {
         rfo.reward_balance += asset( r, ODYSSEY_SYMBOL );
      });

      used_rewards += r;

      // Sanity check to ensure we aren't printing more ODYSSEY than has been allocated through inflation
 ...
 *  the head block time and then converts them to/from odyssey/sbd at the
...
void database::process_conversions()
{
   ...
   asset net_odyssey( 0, ODYSSEY_SYMBOL );
   ...
   modify( props, [&]( dynamic_global_property_object& p )
   {
       p.current_supply += net_odyssey;
       p.current_sbd_supply -= net_sbd;
       p.virtual_supply += net_odyssey;
       p.virtual_supply -= net_sbd * get_feed_history().current_median_history;
   } );
}

asset database::to_sbd( const asset& odyssey )const
{
   return util::to_sbd( get_feed_history().current_median_history, odyssey );
}

asset database::to_odyssey( const asset& sbd )const
{
   return util::to_odyssey( get_feed_history().current_median_history, sbd );
}
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + ODYSSEY_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
void database::expire_escrow_ratification()
{
   ...
   while( escrow_itr != escrow_idx.end() && !escrow_itr->is_approved() && escrow_itr->ratification_deadline <= head_block_time() )
   {
      ...
      adjust_balance( from_account, old_escrow.odyssey_balance );
...
void database::process_decline_voting_rights()
{
   ...
   while( itr != request_idx.end() && itr->effective_date <= head_block_time() )
   {
      ...
      std::array<share_type, ODYSSEY_MAX_PROXY_RECURSION_DEPTH+1> delta;
      delta[0] = -account.vesting_shares.amount;
      for( int i = 0; i < ODYSSEY_MAX_PROXY_RECURSION_DEPTH; ++i )
      ...
      {
         a.can_vote = false;
         a.proxy = ODYSSEY_PROXY_TO_SELF_ACCOUNT;
      });
...
void database::init_genesis( uint64_t init_supply )
{
   try
   {
      ...
      public_key_type      init_public_key(ODYSSEY_INIT_PUBLIC_KEY);

      create< account_object >( [&]( account_object& a )
      {
         a.name = ODYSSEY_MINER_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = ODYSSEY_MINER_ACCOUNT;
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = ODYSSEY_NULL_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = ODYSSEY_NULL_ACCOUNT;
         auth.owner.weight_threshold = 1;
         auth.active.weight_threshold = 1;
      });
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = ODYSSEY_TEMP_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = ODYSSEY_TEMP_ACCOUNT;
         auth.owner.weight_threshold = 0;
         auth.active.weight_threshold = 0;
      });

      for( int i = 0; i < ODYSSEY_NUM_INIT_MINERS; ++i )
      {
         create< account_object >( [&]( account_object& a )
         {
            a.name = ODYSSEY_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
            a.memo_key = init_public_key;
            a.balance  = asset( i ? 0 : init_supply, ODYSSEY_SYMBOL );
         } );

         create< account_authority_object >( [&]( account_authority_object& auth )
         {
            auth.account = ODYSSEY_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         {
            w.owner        = ODYSSEY_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      {
         p.current_witness = ODYSSEY_INIT_MINER_NAME;
         p.time = ODYSSEY_GENESIS_TIME;
         p.recent_slots_filled = fc::uint128::max_value();
         p.participation_count = 128;
         p.current_supply = asset( init_supply, ODYSSEY_SYMBOL );
         p.virtual_supply = p.current_supply;
         p.maximum_block_size = ODYSSEY_MAX_BLOCK_SIZE;
      } );
      ...
      {
         hpo.processed_hardforks.push_back( ODYSSEY_GENESIS_TIME );
      } );

      // Create witness scheduler
      create< witness_schedule_object >( [&]( witness_schedule_object& wso )
      {
         wso.current_shuffled_witnesses[0] = ODYSSEY_INIT_MINER_NAME;
      } );
   }
   FC_CAPTURE_AND_RETHROW()
}
...
void database::notify_changed_objects()
{
   try
   {
      ...
      get_changed_ids( ids );
      ODYSSEY_TRY_NOTIFY( changed_objects, ids )*/
      /*
      if( _undo_db.enabled() )
      {
         ...
         ODYSSEY_TRY_NOTIFY( changed_objects, changed_ids )
      }
      */
   }
   FC_CAPTURE_AND_RETHROW()
...
{ try {
   ...
   if( has_hardfork( ODYSSEY_HARDFORK_0_12 ) )
   {
   ...
   if( block_size < ODYSSEY_MIN_BLOCK_SIZE )
   {
      elog( "Block size is too small",
         ("next_block_num",next_block_num)("block_size", block_size)("min",ODYSSEY_MIN_BLOCK_SIZE)
      );
   }
   ...
   if( has_hardfork( ODYSSEY_HARDFORK_0_5__54 ) ) // Cannot remove after hardfork
   {
...
try {
   if( (head_block_num() % ODYSSEY_FEED_INTERVAL_BLOCKS) != 0 )
      return;
   ...
   for( int i = 0; i < wso.num_scheduled_witnesses; i++ )
   {
      ...
      if( has_hardfork( ODYSSEY_HARDFORK_0_19__822 ) )
      {
         if( now < wit.last_sbd_exchange_update + ODYSSEY_MAX_FEED_AGE_SECONDS
            && !wit.sbd_exchange_rate.is_null() )
         {
            feeds.push_back( wit.sbd_exchange_rate );
         }
      }
      else if( wit.last_sbd_exchange_update < now + ODYSSEY_MAX_FEED_AGE_SECONDS &&
          !wit.sbd_exchange_rate.is_null() )
      {
         feeds.push_back( wit.sbd_exchange_rate );
      }
   ...
   if( feeds.size() >= ODYSSEY_MIN_FEEDS )
   {
      ...
      modify( get_feed_history(), [&]( feed_history_object& fho )
      {
         ...
         size_t odyssey_feed_history_window = ODYSSEY_FEED_HISTORY_WINDOW_PRE_HF_16;
         if( has_hardfork( ODYSSEY_HARDFORK_0_16__551) )
            odyssey_feed_history_window = ODYSSEY_FEED_HISTORY_WINDOW;

         if( fho.price_history.size() > odyssey_feed_history_window )
         ...
         if( fho.price_history.size() )
         {
            ...
            if( skip_price_feed_limit_check )
               return;
#endif
            if( has_hardfork( ODYSSEY_HARDFORK_0_14__230 ) )
            {
               const auto& gpo = get_dynamic_global_properties();
...
{ try {
   ...
   const chain_id_type& chain_id = ODYSSEY_CHAIN_ID;
   auto trx_id = trx.id();
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, ODYSSEY_MAX_SIG_CHECK_DEPTH );
      }
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      if( !(skip & skip_tapos_check) )
      {
         ...
         ODYSSEY_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
      ...
      ODYSSEY_ASSERT( trx.expiration <= now + fc::seconds(ODYSSEY_MAX_TIME_UNTIL_EXPIRATION), transaction_expiration_exception,
                  "", ("trx.expiration",trx.expiration)("now",now)("max_til_exp",ODYSSEY_MAX_TIME_UNTIL_EXPIRATION));
      if( has_hardfork( ODYSSEY_HARDFORK_0_9 ) ) // Simple solution to pending trx bug when now == trx.expiration
         ODYSSEY_ASSERT( now < trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
      ODYSSEY_ASSERT( now <= trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
   }
...
{ try {
   ...
   if( head_block_time() != fc::time_point_sec() )
   {
      ...
      for( uint32_t i = 0; i < missed_blocks; ++i )
      {
         ...
         if(  witness_missed.owner != b.witness )
         {
            ...
            {
               w.total_missed++;
               if( has_hardfork( ODYSSEY_HARDFORK_0_14__278 ) )
               {
                  if( head_block_num() - w.last_confirmed_block_num  > ODYSSEY_BLOCKS_PER_DAY )
                  {
   ...
   {
      ODYSSEY_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < ODYSSEY_MAX_UNDO_HISTORY, undo_database_exception,
                 "The database does not have enough undo history to support a blockchain with so many missed blocks. "
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",ODYSSEY_MAX_UNDO_HISTORY) );
   }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
   modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& dgp )
   {
      dgp.virtual_supply = dgp.current_supply
         + ( get_feed_history().current_median_history.is_null() ? asset( 0, ODYSSEY_SYMBOL ) : dgp.current_sbd_supply * get_feed_history().current_median_history );

      auto median_price = get_feed_history().current_median_history;

      if( !median_price.is_null() && has_hardfork( ODYSSEY_HARDFORK_0_14__230 ) )
      {
         auto percent_sbd = uint16_t( ( ( fc::uint128_t( ( dgp.current_sbd_supply * get_feed_history().current_median_history ).amount.value ) * ODYSSEY_100_PERCENT )
            / dgp.virtual_supply.amount.value ).to_uint64() );

         if( percent_sbd <= ODYSSEY_SBD_START_PERCENT )
            dgp.sbd_print_rate = ODYSSEY_100_PERCENT;
         else if( percent_sbd >= ODYSSEY_SBD_STOP_PERCENT )
            dgp.sbd_print_rate = 0;
         else
            dgp.sbd_print_rate = ( ( ODYSSEY_SBD_STOP_PERCENT - percent_sbd ) * ODYSSEY_100_PERCENT ) / ( ODYSSEY_SBD_STOP_PERCENT - ODYSSEY_SBD_START_PERCENT );
      }
   });
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
    ...
    *
    */
   if( head_block_num() < ODYSSEY_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > ODYSSEY_MAX_WITNESSES )
            _dpo.last_irreversible_block_num = head_block_num() - ODYSSEY_MAX_WITNESSES;
      } );
   }
   else
   {
      ...
      static_assert( ODYSSEY_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

      // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
      // 1 1 1 1 1 1 1 2 2 2 -> 1
      // 3 3 3 3 3 3 3 3 3 3 -> 3

      size_t offset = ((ODYSSEY_100_PERCENT - ODYSSEY_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / ODYSSEY_100_PERCENT);
...
int database::match( const limit_order_object& new_order, const limit_order_object& old_order, const price& match_price )
{
   ...
   if( !has_hardfork( ODYSSEY_HARDFORK_0_12__178 ) &&
       ( (age >= ODYSSEY_MIN_LIQUIDITY_REWARD_PERIOD_SEC && !has_hardfork( ODYSSEY_HARDFORK_0_10__149)) ||
       (age >= ODYSSEY_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10 && has_hardfork( ODYSSEY_HARDFORK_0_10__149) ) ) )
   {
      if( old_order_receives.symbol == ODYSSEY_SYMBOL )
      {
...
void database::adjust_liquidity_reward( const account_object& owner, const asset& volume, bool is_sdb )
{
   ...
   if( itr != ridx.end() )
   {
      ...
      {
         if( head_block_time() - r.last_update >= ODYSSEY_LIQUIDITY_TIMEOUT_SEC )
         {
            r.sbd_volume = 0;
            r.odyssey_volume = 0;
            r.weight = 0;
         ...
         if( is_sdb )
            ...
            r.odyssey_volume += volume.amount.value;

         r.update_weight( has_hardfork( ODYSSEY_HARDFORK_0_10__141 ) );
   ...
   else
   {
      create<liquidity_reward_balance_object>( [&](liquidity_reward_balance_object& r )
      {
         ...
         r.update_weight( has_hardfork( ODYSSEY_HARDFORK_0_9__141 ) );
         r.last_update = head_block_time();
      } );
   }
}
         ...
         case SBD_SYMBOL:
            if( a.sbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.sbd_seconds > 0 &&
                   (acnt.sbd_seconds_last_update - acnt.sbd_last_interest_payment).to_seconds() > ODYSSEY_SBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.sbd_seconds / ODYSSEY_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().sbd_interest_rate;
                  interest /= ODYSSEY_100_PERCENT;
         ...
         case SBD_SYMBOL:
            if( a.savings_sbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.savings_sbd_seconds > 0 &&
                   (acnt.savings_sbd_seconds_last_update - acnt.savings_sbd_last_interest_payment).to_seconds() > ODYSSEY_SBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.savings_sbd_seconds / ODYSSEY_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().sbd_interest_rate;
                  interest /= ODYSSEY_100_PERCENT;
                  asset interest_paid(interest.to_uint64(), SBD_SYMBOL);
...
void database::adjust_reward_balance( const account_object& a, const asset& delta )
{
   modify( a, [&]( account_object& acnt )
   {
      ...
      {
         case ODYSSEY_SYMBOL:
            acnt.reward_odyssey_balance += delta;
            break;
         case SBD_SYMBOL:
...
void database::adjust_supply( const asset& delta, bool adjust_vesting )
{
   ...
   const auto& props = get_dynamic_global_properties();
   if( props.head_block_number < ODYSSEY_BLOCKS_PER_DAY*7 )
   ...
   modify( props, [&]( dynamic_global_property_object& props )
   {
      switch( delta.symbol )
      {
         case ODYSSEY_SYMBOL:
         {
            asset new_vesting( (adjust_vesting && delta.amount > 0) ? delta.amount * 9 : 0, ODYSSEY_SYMBOL );
            props.current_supply += delta + new_vesting;
            props.virtual_supply += delta + new_vesting;
            props.total_vesting_fund_odyssey += new_vesting;
...
{
   _hardfork_times[ 0 ] = fc::time_point_sec( ODYSSEY_GENESIS_TIME );
   _hardfork_versions[ 0 ] = hardfork_version( 0, 0 );
   FC_ASSERT( ODYSSEY_HARDFORK_0_1 == 1, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_1 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_1_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_1 ] = ODYSSEY_HARDFORK_0_1_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_2 == 2, "Invlaid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_2 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_2_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_2 ] = ODYSSEY_HARDFORK_0_2_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_3 == 3, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_3 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_3_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_3 ] = ODYSSEY_HARDFORK_0_3_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_4 == 4, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_4 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_4_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_4 ] = ODYSSEY_HARDFORK_0_4_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_5 == 5, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_5 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_5_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_5 ] = ODYSSEY_HARDFORK_0_5_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_6 == 6, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_6 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_6_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_6 ] = ODYSSEY_HARDFORK_0_6_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_7 == 7, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_7 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_7_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_7 ] = ODYSSEY_HARDFORK_0_7_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_8 == 8, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_8 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_8_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_8 ] = ODYSSEY_HARDFORK_0_8_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_9 == 9, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_9 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_9_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_9 ] = ODYSSEY_HARDFORK_0_9_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_10 == 10, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_10 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_10_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_10 ] = ODYSSEY_HARDFORK_0_10_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_11 == 11, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_11 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_11_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_11 ] = ODYSSEY_HARDFORK_0_11_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_12 == 12, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_12 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_12_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_12 ] = ODYSSEY_HARDFORK_0_12_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_13 == 13, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_13 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_13_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_13 ] = ODYSSEY_HARDFORK_0_13_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_14 == 14, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_14 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_14_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_14 ] = ODYSSEY_HARDFORK_0_14_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_15 == 15, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_15 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_15_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_15 ] = ODYSSEY_HARDFORK_0_15_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_16 == 16, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_16 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_16_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_16 ] = ODYSSEY_HARDFORK_0_16_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_17 == 17, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_17 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_17_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_17 ] = ODYSSEY_HARDFORK_0_17_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_18 == 18, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_18 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_18_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_18 ] = ODYSSEY_HARDFORK_0_18_VERSION;
   FC_ASSERT( ODYSSEY_HARDFORK_0_19 == 19, "Invalid hardfork configuration" );
   _hardfork_times[ ODYSSEY_HARDFORK_0_19 ] = fc::time_point_sec( ODYSSEY_HARDFORK_0_19_TIME );
   _hardfork_versions[ ODYSSEY_HARDFORK_0_19 ] = ODYSSEY_HARDFORK_0_19_VERSION;


   const auto& hardforks = get_hardfork_property_object();
   FC_ASSERT( hardforks.last_hardfork <= ODYSSEY_NUM_HARDFORKS, "Chain knows of more hardforks than configuration", ("hardforks.last_hardfork",hardforks.last_hardfork)("ODYSSEY_NUM_HARDFORKS",ODYSSEY_NUM_HARDFORKS) );
   FC_ASSERT( _hardfork_versions[ hardforks.last_hardfork ] <= ODYSSEY_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
   FC_ASSERT( ODYSSEY_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions[ ODYSSEY_NUM_HARDFORKS ] );
}
...
void database::process_hardforks()
{
   try
   {
      ...
      if( has_hardfork( ODYSSEY_HARDFORK_0_5__54 ) )
      {
         ...
         {
            if( hardforks.last_hardfork < ODYSSEY_NUM_HARDFORKS ) {
      ...
      {
         while( hardforks.last_hardfork < ODYSSEY_NUM_HARDFORKS
               && _hardfork_times[ hardforks.last_hardfork + 1 ] <= head_block_time()
               && hardforks.last_hardfork < ODYSSEY_HARDFORK_0_5__54 )
         {
            apply_hardfork( hardforks.last_hardfork + 1 );
         }
      }
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
   ...
   for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= ODYSSEY_NUM_HARDFORKS; i++ )
   {
      if( i <= ODYSSEY_HARDFORK_0_5__54 )
   ...
   switch( hardfork )
   {
      case ODYSSEY_HARDFORK_0_1:
...
#ifdef IS_TEST_NET
         {
            ...
            test_op.required_auths.insert( ODYSSEY_INIT_MINER_NAME );
...
#endif
         break;
      case ODYSSEY_HARDFORK_0_2:
         retally_witness_votes();
         break;
      case ODYSSEY_HARDFORK_0_3:
         retally_witness_votes();
         break;
      case ODYSSEY_HARDFORK_0_4:
         reset_virtual_schedule_time(*this);
         break;
      case ODYSSEY_HARDFORK_0_5:
         break;
      case ODYSSEY_HARDFORK_0_6:
         retally_witness_vote_counts();
         retally_comment_children();
         break;
      case ODYSSEY_HARDFORK_0_7:
         break;
      case ODYSSEY_HARDFORK_0_8:
         retally_witness_vote_counts(true);
         break;
      case ODYSSEY_HARDFORK_0_9:
         {
         ...
         break;
      case ODYSSEY_HARDFORK_0_10:
         retally_liquidity_weight();
         break;
      case ODYSSEY_HARDFORK_0_11:
         break;
      case ODYSSEY_HARDFORK_0_12:
         {
            ...
            for( auto itr = comment_idx.begin(); itr != comment_idx.end(); ++itr )
            {
               ...
               if( itr->parent_author == ODYSSEY_ROOT_POST_PARENT )
               {
                  ...
                  if( itr->last_payout == fc::time_point_sec::min() && itr->cashout_time == fc::time_point_sec::maximum() )
                  {
                     ...
                     {
                        c.cashout_time = head_block_time() + ODYSSEY_CASHOUT_WINDOW_SECONDS_PRE_HF17;
                     });
                  }
                  ...
                  else if( itr->last_payout > fc::time_point_sec() )
                  {
                     modify( *itr, [&]( comment_object& c )
                     {
                        c.cashout_time = c.last_payout + ODYSSEY_SECOND_CASHOUT_WINDOW;
                     });
                  }
               }
            }

            modify( get< account_authority_object, by_account >( ODYSSEY_MINER_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( ODYSSEY_NULL_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( ODYSSEY_TEMP_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });
         ...
         break;
      case ODYSSEY_HARDFORK_0_13:
         break;
      case ODYSSEY_HARDFORK_0_14:
         break;
      case ODYSSEY_HARDFORK_0_15:
         break;
      case ODYSSEY_HARDFORK_0_16:
         {
            modify( get_feed_history(), [&]( feed_history_object& fho )
            {
               while( fho.price_history.size() > ODYSSEY_FEED_HISTORY_WINDOW )
                  fho.price_history.pop_front();
            });
         }
         break;
      case ODYSSEY_HARDFORK_0_17:
         {
            static_assert(
               ODYSSEY_MAX_VOTED_WITNESSES_HF0 + ODYSSEY_MAX_MINER_WITNESSES_HF0 + ODYSSEY_MAX_RUNNER_WITNESSES_HF0 == ODYSSEY_MAX_WITNESSES,
               "HF0 witness counts must add up to ODYSSEY_MAX_WITNESSES" );
            static_assert(
               ODYSSEY_MAX_VOTED_WITNESSES_HF17 + ODYSSEY_MAX_MINER_WITNESSES_HF17 + ODYSSEY_MAX_RUNNER_WITNESSES_HF17 == ODYSSEY_MAX_WITNESSES,
               "HF17 witness counts must add up to ODYSSEY_MAX_WITNESSES" );

            modify( get_witness_schedule_object(), [&]( witness_schedule_object& wso )
            {
               wso.max_voted_witnesses = ODYSSEY_MAX_VOTED_WITNESSES_HF17;
               wso.max_miner_witnesses = ODYSSEY_MAX_MINER_WITNESSES_HF17;
               wso.max_runner_witnesses = ODYSSEY_MAX_RUNNER_WITNESSES_HF17;
            });
            ...
            {
               rfo.name = ODYSSEY_POST_REWARD_FUND_NAME;
               rfo.last_update = head_block_time();
               rfo.content_constant = ODYSSEY_CONTENT_CONSTANT_HF0;
               rfo.percent_curation_rewards = ODYSSEY_1_PERCENT * 25;
               rfo.percent_content_rewards = ODYSSEY_100_PERCENT;
               rfo.reward_balance = gpo.total_reward_fund_odyssey;
#ifndef IS_TEST_NET
               rfo.recent_claims = ODYSSEY_HF_17_RECENT_CLAIMS;
#endif
            ...
            modify( gpo, [&]( dynamic_global_property_object& g )
            {
               g.total_reward_fund_odyssey = asset( 0, ODYSSEY_SYMBOL );
            ...
            */
            ...
            root_posts.reserve( ODYSSEY_HF_17_NUM_POSTS );
            vector< const comment_object* > replies;
            replies.reserve( ODYSSEY_HF_17_NUM_REPLIES );
            ...
            for( auto itr : root_posts )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( c.created + ODYSSEY_CASHOUT_WINDOW_SECONDS, c.cashout_time );
               });
            }
            ...
            for( auto itr : replies )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( calculate_discussion_payout_time( c ), c.created + ODYSSEY_CASHOUT_WINDOW_SECONDS );
               });
            }
         }
         break;
      case ODYSSEY_HARDFORK_0_18:
         break;
      case ODYSSEY_HARDFORK_0_19:
         {
            ...
            modify( get< reward_fund_object, by_name >( ODYSSEY_POST_REWARD_FUND_NAME ), [&]( reward_fund_object &rfo )
            {
#ifndef IS_TEST_NET
               rfo.recent_claims = ODYSSEY_HF_19_RECENT_CLAIMS;
#endif
...
void database::validate_invariants()const
{
   try
   {
      ...
      asset total_supply = asset( 0, ODYSSEY_SYMBOL );
      asset total_sbd = asset( 0, SBD_SYMBOL );
      asset total_vesting = asset( 0, VESTS_SYMBOL );
      asset pending_vesting_odyssey = asset( 0, ODYSSEY_SYMBOL );
      ...
      for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
      {
         ...
         total_supply += itr->reward_odyssey_balance;
         ...
         pending_vesting_odyssey += itr->reward_vesting_odyssey;
         total_vsf_votes += ( itr->proxy == ODYSSEY_PROXY_TO_SELF_ACCOUNT ?
                                 itr->witness_vote_weight() :
                                 ( ODYSSEY_MAX_PROXY_RECURSION_DEPTH > 0 ?
                                      itr->proxied_vsf_votes[ODYSSEY_MAX_PROXY_RECURSION_DEPTH - 1] :
      ...
      {
         if( itr->amount.symbol == ODYSSEY_SYMBOL )
      ...
      {
         if( itr->sell_price.base.symbol == ODYSSEY_SYMBOL )
         {
            total_supply += asset( itr->for_sale, ODYSSEY_SYMBOL );
         }
      ...
      {
         total_supply += itr->odyssey_balance;
         total_sbd += itr->sbd_balance;

         if( itr->pending_fee.symbol == ODYSSEY_SYMBOL )
            total_supply += itr->pending_fee;
         else if( itr->pending_fee.symbol == SBD_SYMBOL )
            total_sbd += itr->pending_fee;
         else
            FC_ASSERT( false, "found escrow pending fee that is not SBD or ODYSSEY" );
      }
      ...
      for( auto itr = savings_withdraw_idx.begin(); itr != savings_withdraw_idx.end(); ++itr )
      {
         if( itr->amount.symbol == ODYSSEY_SYMBOL )
            ...
            FC_ASSERT( false, "found savings withdraw that is not SBD or ODYSSEY" );
      }
      ...
      total_supply += gpo.total_vesting_fund_odyssey + gpo.total_reward_fund_odyssey + gpo.pending_rewarded_vesting_odyssey;
      ...
      FC_ASSERT( gpo.pending_rewarded_vesting_odyssey == pending_vesting_odyssey, "", ("pending_rewarded_vesting_odyssey",gpo.pending_rewarded_vesting_odyssey)("pending_vesting_odyssey", pending_vesting_odyssey));
   ...
   try
   {
      ...
      for( const auto& account : get_index<account_index>().indices() )
      {
         modify( account, [&]( account_object& a )
         {
            ...
            a.vesting_withdraw_rate  = asset( a.to_withdraw / ODYSSEY_VESTING_WITHDRAW_INTERVALS_PRE_HF_16, VESTS_SYMBOL );
            if( a.vesting_withdraw_rate.amount == 0 )
               a.vesting_withdraw_rate.amount = 1;

            for( uint32_t i = 0; i < ODYSSEY_MAX_PROXY_RECURSION_DEPTH; ++i )
...
// Low memory nodes only need immediate child count, full nodes track total children
         ...
         while( parent )
         {
            ...
            if( parent->parent_author != ODYSSEY_ROOT_POST_PARENT )
...
void database::retally_witness_votes()
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      if( itr->proxy != ODYSSEY_PROXY_TO_SELF_ACCOUNT ) continue;
...
void database::retally_witness_vote_counts( bool force )
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      ...
      if( force || (a.proxy != ODYSSEY_PROXY_TO_SELF_ACCOUNT  ) )
      {
        const auto& vidx = get_index< witness_vote_index >().indices().get< by_account_witness >();
        auto wit_itr = vidx.lower_bound( boost::make_tuple( a.id, witness_id_type() ) );
        while( wit_itr != vidx.end() && wit_itr->account == a.id )
