...
using odyssey::protocol::block_id_type;
using odyssey::protocol::transaction_id_type;
using odyssey::protocol::chain_id_type;
using odyssey::protocol::account_name_type;
...
{
   class variant;
   inline void to_variant( const odyssey::chain::shared_string& s, variant& var )
   {
      var = fc::string( odyssey::chain::to_string( s ) );
   }

   inline void from_variant( const variant& var, odyssey::chain::shared_string& s )
   {
   ...
   namespace raw
   {
      ...
      template< typename T > inline void pack( odyssey::chain::buffer_type& raw, const T& v )
      {
      ...
      template< typename T > inline void unpack( const odyssey::chain::buffer_type& raw, T& v )
      {
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v );
      }

      template< typename T > inline T unpack( const odyssey::chain::buffer_type& raw )
      {
         T v;
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v );
...
FC_REFLECT_ENUM( odyssey::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_authority_object_type)
                 (witness_object_type)
...
FC_REFLECT_TYPENAME( odyssey::chain::shared_string )
FC_REFLECT_TYPENAME( odyssey::chain::buffer_type )

FC_REFLECT_ENUM( odyssey::chain::bandwidth_type, (post)(forum)(market) )
