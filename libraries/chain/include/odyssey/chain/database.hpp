...
#include <odyssey/chain/hardfork.hpp>
#include <odyssey/chain/node_property_object.hpp>
#include <odyssey/chain/fork_database.hpp>
#include <odyssey/chain/block_log.hpp>
#include <odyssey/chain/operation_notification.hpp>

#include <odyssey/protocol/protocol.hpp>
...
namespace odyssey { namespace chain {
   ...
   using odyssey::protocol::signed_transaction;
   using odyssey::protocol::operation;
   ...
   class database : public chainbase::database
   {
      public:
          ...
          */
         void open( const fc::path& data_dir, const fc::path& shared_mem_dir, uint64_t initial_supply = ODYSSEY_INIT_SUPPLY, uint64_t shared_file_size = 0, uint32_t chainbase_flags = 0 );
          ...
          *
          * Passing slot_num == 0 returns ODYSSEY_NULL_WITNESS
          */
         ...
         /** @return the sbd created and deposited to_account, may return ODYSSEY if there is no median feed */
         std::pair< asset, asset > create_sbd( const account_object& to_account, asset odyssey, bool to_reward_balance=false );
         asset create_vesting( const account_object& to_account, asset odyssey, bool to_reward_balance=false );
          ...
          * ODYSSEY.  Return 0 SBD if there isn't a current_median_history
          */
         asset to_sbd( const asset& odyssey )const;
         asset to_odyssey( const asset& sbd )const;
         ...
         void init_schema();
         void init_genesis(uint64_t initial_supply = ODYSSEY_INIT_SUPPLY );
   ...
   protected:
      ...
      private:
         ...
         fc::time_point_sec            _hardfork_times[ ODYSSEY_NUM_HARDFORKS + 1 ];
         protocol::hardfork_version    _hardfork_versions[ ODYSSEY_NUM_HARDFORKS + 1 ];
