...
namespace odyssey { namespace chain {
   ...
   class dynamic_global_property_object : public object< dynamic_global_property_object_type, dynamic_global_property_object>
   {
      public:
         ...
         asset       virtual_supply             = asset( 0, ODYSSEY_SYMBOL );
         asset       current_supply             = asset( 0, ODYSSEY_SYMBOL );
         asset       confidential_supply        = asset( 0, ODYSSEY_SYMBOL ); ///< total asset held in confidential balances
         ...
         asset       total_vesting_fund_odyssey   = asset( 0, ODYSSEY_SYMBOL );
         asset       total_vesting_shares       = asset( 0, VESTS_SYMBOL );
         asset       total_reward_fund_odyssey    = asset( 0, ODYSSEY_SYMBOL );
         fc::uint128 total_reward_shares2; ///< the running total of REWARD^2
         asset       pending_rewarded_vesting_shares = asset( 0, VESTS_SYMBOL );
         asset       pending_rewarded_vesting_odyssey = asset( 0, ODYSSEY_SYMBOL );

         price       get_vesting_share_price() const
         {
            if ( total_vesting_fund_odyssey.amount == 0 || total_vesting_shares.amount == 0 )
               return price ( asset( 1000, ODYSSEY_SYMBOL ), asset( 1000000, VESTS_SYMBOL ) );

            return price( total_vesting_shares, total_vesting_fund_odyssey );
         }

         price get_reward_vesting_share_price() const
         {
            return price( total_vesting_shares + pending_rewarded_vesting_shares,
               total_vesting_fund_odyssey + pending_rewarded_vesting_odyssey );
         }
         ...
         uint16_t sbd_print_rate = ODYSSEY_100_PERCENT;
...
FC_REFLECT( odyssey::chain::dynamic_global_property_object,
             (id)
             (head_block_number)
             (head_block_id)
             (time)
             ...
             (total_vesting_fund_odyssey)
             (total_vesting_shares)
             (total_reward_fund_odyssey)
             (total_reward_shares2)
             (pending_rewarded_vesting_shares)
             (pending_rewarded_vesting_odyssey)
             (sbd_interest_rate)
             (sbd_print_rate)
             (maximum_block_size)
             (current_aslot)
...
CHAINBASE_SET_INDEX_TYPE( odyssey::chain::dynamic_global_property_object, odyssey::chain::dynamic_global_property_index )
