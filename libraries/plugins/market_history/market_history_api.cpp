#include <odyssey/market_history/market_history_api.hpp>

#include <odyssey/chain/odyssey_objects.hpp>

namespace odyssey { namespace market_history {
...
{
   public:
      market_history_api_impl( odyssey::app::application& _app )
         :app( _app ) {}
...
market_ticker market_history_api_impl::get_ticker() const
{
   ...
   {
      auto open = ( asset( itr->open_sbd, SBD_SYMBOL ) / asset( itr->open_odyssey, ODYSSEY_SYMBOL ) ).to_real();
      result.latest = ( asset( itr->close_sbd, SBD_SYMBOL ) / asset( itr->close_odyssey, ODYSSEY_SYMBOL ) ).to_real();
   ...
   result.odyssey_volume = volume.odyssey_volume;
...
market_volume market_history_api_impl::get_volume() const
{
   ...
   do
   {
      result.odyssey_volume.amount += itr->odyssey_volume;
...
order_book market_history_api_impl::get_order_book( uint32_t limit ) const
{
   ...
   const auto& order_idx = app.chain_database()->get_index< odyssey::chain::limit_order_index >().indices().get< odyssey::chain::by_price >();
   auto itr = order_idx.lower_bound( price::max( SBD_SYMBOL, ODYSSEY_SYMBOL ) );
   ...
   while( itr != order_idx.end() && itr->sell_price.base.symbol == SBD_SYMBOL && result.bids.size() < limit )
   {
      ...
      cur.odyssey = ( asset( itr->for_sale, SBD_SYMBOL ) * itr->sell_price ).amount;
      cur.sbd = itr->for_sale;
      result.bids.push_back( cur );
      ++itr;
   }

   itr = order_idx.lower_bound( price::max( ODYSSEY_SYMBOL, SBD_SYMBOL ) );

   while( itr != order_idx.end() && itr->sell_price.base.symbol == ODYSSEY_SYMBOL && result.asks.size() < limit )
   {
      order cur;
      cur.price = itr->sell_price.quote.to_real() / itr->sell_price.base.to_real();
      cur.odyssey = itr->for_sale;
      cur.sbd = ( asset( itr->for_sale, ODYSSEY_SYMBOL ) * itr->sell_price ).amount;
...
market_history_api::market_history_api( const odyssey::app::api_context& ctx )
{
