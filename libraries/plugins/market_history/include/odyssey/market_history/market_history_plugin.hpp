...
class market_history_plugin : public odyssey::app::plugin
{
   public:
...
struct bucket_object : public object< bucket_object_type, bucket_object >
{
   ...
   uint32_t             seconds = 0;
   share_type           high_odyssey;
   share_type           high_sbd;
   share_type           low_odyssey;
   share_type           low_sbd;
   share_type           open_odyssey;
   share_type           open_sbd;
   share_type           close_odyssey;
   share_type           close_sbd;
   share_type           odyssey_volume;
   share_type           sbd_volume;

   price high()const { return asset( high_sbd, SBD_SYMBOL ) / asset( high_odyssey, ODYSSEY_SYMBOL ); }
   price low()const { return asset( low_sbd, SBD_SYMBOL ) / asset( low_odyssey, ODYSSEY_SYMBOL ); }
};
...
FC_REFLECT( odyssey::market_history::bucket_object,
                     (id)
                     (open)(seconds)
                     (high_odyssey)(high_sbd)
                     (low_odyssey)(low_sbd)
                     (open_odyssey)(open_sbd)
                     (close_odyssey)(close_sbd)
                     (odyssey_volume)(sbd_volume) )
CHAINBASE_SET_INDEX_TYPE( odyssey::market_history::bucket_object, odyssey::market_history::bucket_index )

FC_REFLECT( odyssey::market_history::order_history_object,
                     (id)
                     (time)
                     (op) )
CHAINBASE_SET_INDEX_TYPE( odyssey::market_history::order_history_object, odyssey::market_history::order_history_index )
