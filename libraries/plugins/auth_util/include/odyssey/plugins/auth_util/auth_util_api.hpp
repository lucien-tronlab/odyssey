...
{
   public:
      auth_util_api( const odyssey::app::api_context& ctx );
...
FC_REFLECT( odyssey::plugin::auth_util::check_authority_signature_params,
   (account_name)
   (level)
   (dig)
   (sigs)
   )
FC_REFLECT( odyssey::plugin::auth_util::check_authority_signature_result,
   (keys)
   )

FC_API( odyssey::plugin::auth_util::auth_util_api,
