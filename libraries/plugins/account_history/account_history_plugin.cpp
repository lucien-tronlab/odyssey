#include <odyssey/account_history/account_history_plugin.hpp>
...
#define ODYSSEY_NAMESPACE_PREFIX "odyssey::protocol::"

namespace odyssey { namespace account_history {
...
void account_history_plugin_impl::on_operation( const operation_notification& note )
{
   ...
   odyssey::chain::database& db = database();
...
void account_history_plugin::plugin_initialize(const boost::program_options::variables_map& options)
{
   ...
   if( options.count( "history-whitelist-ops" ) )
   {
      ...
      for( auto& arg : options.at( "history-whitelist-ops" ).as< vector< string > >() )
      {
         ...
         {
            if( op.size() )
               my->_op_list.insert( ODYSSEY_NAMESPACE_PREFIX + op );
         }
      }
...
ODYSSEY_DEFINE_PLUGIN( account_history, odyssey::account_history::account_history_plugin )
