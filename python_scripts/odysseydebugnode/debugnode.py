...
from odysseyapi.odysseynoderpc import OdysseyNodeRPC

class DebugNode( object ):
   """ Wraps the odysseyd debug node plugin for easier automated testing of the Odyssey Network"""

   def __init__( self, odysseyd, data_dir, args='', plugins=[], apis=[], odysseyd_out=None, odysseyd_err=None ):
      """ Creates a odysseyd debug node.
      ...
      For all other requests, the python-odyssey library should be used.

      args:
         odysseyd -- The string path to the location of the odysseyd binary
         data_dir -- The string path to an existing odysseyd data directory which will be used to pull blocks from.
         args -- Other string args to pass to odysseyd.
         ...
         odysseyd_stdout -- A stream for odysseyd's stdout. Default is to pipe to /dev/null
         odysseyd_stderr -- A stream for odysseyd's stderr. Default is to pipe to /dev/null
      """
      self._data_dir = None
      self._debug_key = None
      self._FNULL = None
      self._rpc = None
      self._odysseyd_bin = None
      self._odysseyd_lock = None
      self._odysseyd_process = None
      self._temp_data_dir = None

      self._odysseyd_bin = Path( odysseyd )
      if( not self._odysseyd_bin.exists() ):
         raise ValueError( 'odysseyd does not exist' )
      if( not self._odysseyd_bin.is_file() ):
         raise ValueError( 'odysseyd is not a file' )

      self._data_dir = Path( data_dir )
      if( not self._data_dir.exists() ):
         raise ValueError( 'data_dir either does not exist or is not a properly constructed odyssey data directory' )
      ...
      if( odysseyd_out != None ):
         self.odysseyd_out = odysseyd_out
      else:
         self.odysseyd_out = self._FNULL

      if( odysseyd_err != None ):
         self.odysseyd_err = odysseyd_err
      else:
         self.odysseyd_err = self._FNULL

      self._debug_key = '5JHNbFNDg834SFj8CMArV6YW7td4zrPzXveqTfaShmYVuYNeK69'
      self._odysseyd_lock = Lock()


   def __enter__( self ):
      self._odysseyd_lock.acquire()
      ...
      odysseyd = [ str( self._odysseyd_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      odysseyd.extend( self._args )

      self._odysseyd_process = Popen( odysseyd, stdout=self.odysseyd_out, stderr=self.odysseyd_err )
      self._odysseyd_process.poll()
      sleep( 5 )
      if( not self._odysseyd_process.returncode ):
         self._rpc = OdysseyNodeRPC( 'ws://127.0.0.1:8095', '', '' )
      else:
         raise Exception( "odysseyd did not start properly..." )

   def __exit__( self, exc, value, tb ):
      self._rpc = None

      if( self._odysseyd_process != None ):
         self._odysseyd_process.poll()

         if( not self._odysseyd_process.returncode ):
            self._odysseyd_process.send_signal( SIGINT )

            sleep( 7 )
            self._odysseyd_process.poll()

            if( not self._odysseyd_process.returncode ):
               self._odysseyd_process.send_signal( SIGTERM )

               sleep( 5 )
               self._odysseyd_process.poll()

               if( self._odysseyd_process.returncode ):
                  loggin.error( 'odysseyd did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )

      self._odysseyd_process = None
      self._temp_data_dir.cleanup()
      self._temp_data_dir = None
      self._odysseyd_lock.release()
   ...
   def debug_generate_blocks( self, count ):
      ...
      `get_dev_key odyssey debug`. Do not use this key on the live chain for any reason.
   ...
   def debug_set_hardfork( self, hardfork_id ):
      ...
      args:
         hardfork_id: The id of the hardfork to set. Hardfork IDs start at 1 (0 is genesis) and increment
            by one for each hardfork. The maximum value is ODYSSEY_NUM_HARDFORKS in chain/hardfork.d/0-preamble.hf
      """
      if( hardfork_id < 0 ):
...
if __name__=="__main__":
   ...
   def main():
      global WAITING
      """
      This example contains a simple parser to obtain the locations of both odysseyd and the data directory,
      ...
      parser = ArgumentParser( description='Run a Debug Node on an existing chain. This simply replays all blocks ' + \
                                 ...
                                 'the CLI wallet' )
      parser.add_argument( '--odysseyd', '-s', type=str, required=True, help='The location of a odysseyd binary to run the debug node' )
      ...
      odysseyd = Path( args.odysseyd )
      if( not odysseyd.exists() ):
         print( 'Error: odysseyd does not exist.' )
         return

      odysseyd = odysseyd.resolve()
      if( not odysseyd.is_file() ):
         print( 'Error: odysseyd is not a file.' )
         return

      data_dir = Path( args.data_dir )
      if( not data_dir.exists() ):
         print( 'Error: data_dir does not exist or is not a properly constructed odysseyd data directory' )
      ...
      debug_node = DebugNode( str( odysseyd ), str( data_dir ), odysseyd_err=sys.stderr )
